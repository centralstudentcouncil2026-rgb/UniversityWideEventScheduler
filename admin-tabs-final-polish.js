(() => {
  if (window.__adminTabsFinalPolish) return;
  window.__adminTabsFinalPolish = true;

  const ADMIN_TABS = ['announcementsModal', 'eventRequestsModal', 'usersModal'];

  function injectStyle() {
    if (document.getElementById('admin-tabs-final-polish-style')) return;
    const style = document.createElement('style');
    style.id = 'admin-tabs-final-polish-style';
    style.textContent = `
      ...
      dialog.admin-tab-fullpage .modal-header>div,
      dialog.portal-tab-view.admin-tab-fullpage .modal-header>div{min-width:0!important;}
      dialog.admin-tab-fullpage .modal-header h3,
      dialog.portal-tab-view.admin-tab-fullpage .modal-header h3{font-size:clamp(1.25rem,2vw,1.8rem)!important;font-weight:900!important;letter-spacing:-.04em!important;}
      dialog.admin-tab-fullpage .modal-header .icon-button[data-close],
      dialog.portal-tab-view.admin-tab-fullpage .modal-header .icon-button[data-close]{display:none!important;}
      ...
      @media(max-width:860px){
        dialog.admin-tab-fullpage .modal-header,dialog.portal-tab-view.admin-tab-fullpage .modal-header{grid-template-columns:1fr!important;align-items:stretch!important;}
  ...
  function closeAdminTab(dialog) {
    dialog?.close?.();
    document.body.classList.remove('portal-tab-open');
    window.CONNECT_STATE?.calendar?.updateSize?.();
  }

  function normalizeHeader(dialog) {
    const header = dialog?.querySelector('.modal-header');
    if (!header) return;
    header.querySelectorAll('.icon-button[data-close]').forEach((button) => { button.hidden = true; button.setAttribute('aria-hidden', 'true'); });
    ...
    if (!back) {
      ...
      back.addEventListener('click', () => closeAdminTab(dialog));
      header.prepend(back);
    }
  ...
  function moveEventFiltersToHeader() {
    const dialog = document.getElementById('eventRequestsModal');
    const filters = document.getElementById('eventRequestFilters');
    const tools = dialog?.querySelector('.admin-tab-header-tools');
  ...
  function splitAnnouncementCards() {
    ...
    if (!form.querySelector('.announcement-input-card')) {
  ...
  function run() {
    ADMIN_TABS.forEach((id) => normalizeHeader(document.getElementById(id)));
  ...
  function init() {
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('#eventRequestsButton,#announcementsButton,#usersButton,#notificationsButton')) setTimeout(run, 180);
    }, true);
  }
