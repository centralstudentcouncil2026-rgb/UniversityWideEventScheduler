import { authenticate, clearSession, loadAuthenticatedStore, requestAccount } from './supabase-storage.js?v=20260624-organization-signup-error-v1';
...
if (signupForm) signupForm.addEventListener('submit', async (event) => {
  ...
  try {
    await requestAccount({ username, password, fullName, organizationName, email: aupEmail, phoneNumber, organizationCode });
    signupForm.reset();
