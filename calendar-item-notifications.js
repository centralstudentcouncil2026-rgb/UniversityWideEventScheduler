...
function cinIsCancellationRequest(event = {}) { return event.revision_status === 'cancel_pending' || event.event_status === 'cancellation_requested'; }
...
function cinIsPendingRequest(event) { return cinIsSchedule(event) && event.approval_status === 'pending' && event.created_by && event.created_by !== cinUser().id; }
function cinIsCreatorResult(event) { return cinIsSchedule(event) && event.created_by === cinUser().id && ['approved', 'rejected'].includes(event.approval_status) && event.notification_status !== 'read'; }

function cinPendingTitle(event) {
  if (!event.revision_of) return 'New Schedule Request';
  return cinIsCancellationRequest(event) ? 'Schedule Removal Requested' : 'Schedule Revision Submitted';
}
...
function cinCreatorTitle(event, approved) {
  if (cinIsCancellationRequest(event)) return `${approved ? 'Approved' : 'Rejected'} Schedule Removal Request`;
  if (event.revision_of) return `${approved ? 'Approved' : 'Rejected'} Schedule Edit Request`;
  return `${approved ? 'Approved' : 'Rejected'} Schedule Request`;
}
function cinCreatorMessage(event) {
  if (cinIsCancellationRequest(event)) return `Your removal request for "${event.title}" was ${event.approval_status}.`;
  if (event.revision_of) return `Your edit request for "${event.title}" was ${event.approval_status}.`;
  return `Your schedule "${event.title}" was ${event.approval_status}.`;
}
  ...
  if (cinIsAdmin()) {
    (store.events || []).filter(cinIsPendingRequest).forEach((event) => {
      ...
      cinUpsertNotification({
        notification_id: `${removal ? 'schedule-removal' : revision ? 'schedule-revision' : 'schedule-request'}-${event.id}-${user.id}`,
        user_id: user.id,
        notification_type: revision ? 'schedule_revision' : 'schedule_update',
        reference_id: event.id,
        title: cinPendingTitle(event),
        message: cinPendingMessage(event),
        is_read: false,
  ...
  if (cinIsManager()) {
    (store.events || []).filter(cinIsCreatorResult).forEach((event) => {
      ...
      cinUpsertNotification({
        ...
        reference_id: event.id,
        title: cinCreatorTitle(event, approved),
        message: cinCreatorMessage(event),
        is_read: false,
...
function cinMarkRead(notification) {
  ...
  if (event && notification.notification_type === 'schedule_approval') event.notification_status = 'read';
  cinUpdateBadge();
}
...
function cinBindClicks() {
  document.addEventListener('click', (clickEvent) => {
    ...
    if (cinIsAdmin() && ['schedule_update', 'schedule_revision'].includes(notification.notification_type)) {
      clickEvent.preventDefault(); clickEvent.stopPropagation(); cinMarkRead(notification);
      const modal = document.getElementById('notificationsModal'); if (modal?.open) modal.close();
