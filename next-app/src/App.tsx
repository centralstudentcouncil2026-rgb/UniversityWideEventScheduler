...
type CalendarFilters = { organization: string; category: string; venue: string; eventType: string; date: string; month: string; status: string };
...
const eventIsActive = (event: CalendarEvent) => !['cancelled', 'completed'].includes(event.event_status);
...
export default function App() {
  ...
  const [filters, setFilters] = useState({ organization: '', category: '', venue: '', eventType: '', date: '', month: '', status: '' });
  ...
  const events = useMemo(() => visibleEvents(store, user).filter((event) => {
    ...
    return (!search || text.includes(search.toLowerCase()))
      && (!filters.organization || event.organization_id === filters.organization)
      && (!filters.category || event.category_id === filters.category)
      && (!filters.venue || event.venue.toLowerCase().includes(filters.venue.toLowerCase()))
      && (!filters.eventType || event.event_type.toLowerCase().includes(filters.eventType.toLowerCase()))
      && (!filters.date || datesFor(event).includes(filters.date))
      && (!filters.month || datesFor(event).some((date) => date.startsWith(filters.month)))
      && (!filters.status || event.event_status === filters.status);
  }), [store, user, search, filters]);
  ...
  useEffect(() => {
    ...
    if (user.role === 'public_viewer' && !['calendar', 'announcements'].includes(workspace)) go('calendar');
    if (user.role !== 'super_admin' && workspace === 'admin') go('calendar');
  }, [user.role, mobile, view, workspace]);
  useEffect(() => {
    const hash = location.hash.slice(1) as Workspace;
    if (['calendar', 'dashboard', 'notifications', 'announcements', 'concerns', 'admin'].includes(hash)) setWorkspace(hash);
  }, []);
  ...
  async function saveEvent(event: CalendarEvent) {
    ...
    const blocked = store.blockedTimes.find((block) => nextEvent.occurrences.some((occurrence) => overlaps(occurrence.start_time, occurrence.end_time, block.start_time, block.end_time)));
    if (blocked) return setNotice(`This date has been blocked by the admin: ${blocked.title}. Please choose another date.`);
    const conflicts = store.events.filter((item) => item.id !== nextEvent.id && eventIsActive(item) && nextEvent.occurrences.some((occurrence) => item.occurrences.some((other) => overlaps(occurrence.start_time, occurrence.end_time, other.start_time, other.end_time))));
    nextEvent.conflict_event_ids = conflicts.map((item) => item.id);
    if (conflicts.length && !confirm(`Warning: ${conflicts.length} other event(s) overlap this schedule. Continue posting anyway?`)) return;
  ...
  return <div className="app">
    ...
    <div className="body">
      <aside className="sidebar">
        ...
        <Filters store={store} filters={filters} setFilters={setFilters} />
      </aside>
      <main className="main">
        ...
        {workspace === 'notifications' && <Notifications store={store} user={user} />}
...
function Nav({ active, go, user }: { active: Workspace; go: (item: Workspace) => void; user: User }) {
  const items: [Workspace, string][] = user.role === 'public_viewer' ? [['calendar', 'Calendar'], ['announcements', 'Announcements']] : [['calendar', 'Calendar'], ['dashboard', 'Dashboard'], ['notifications', 'Notifications'], ['announcements', 'Announcements'], ['concerns', 'Concerns']];
...
function Filters({ store, filters, setFilters }: { store: Store; filters: CalendarFilters; setFilters: (value: CalendarFilters) => void }) {
  const set = (key: keyof CalendarFilters, value: string) => setFilters({ ...filters, [key]: value });
  return <section className="side-note"><strong>Calendar filters</strong><select value={filters.organization} onChange={(event) => set('organization', event.target.value)}><option value="">All organizations</option>{store.organizations.map((item) => <option key={item.id} value={item.id}>{item.organization_name}</option>)}</select><select value={filters.category} onChange={(event) => set('category', event.target.value)}><option value="">All categories</option>{store.categories.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input placeholder="Venue" value={filters.venue} onChange={(event) => set('venue', event.target.value)} /><input placeholder="Event type" value={filters.eventType} onChange={(event) => set('eventType', event.target.value)} /><input type="date" value={filters.date} onChange={(event) => set('date', event.target.value)} /><input type="month" value={filters.month} onChange={(event) => set('month', event.target.value)} /><select value={filters.status} onChange={(event) => set('status', event.target.value)}><option value="">All statuses</option>{['planned', 'finalized', 'postponed', 'cancelled', 'completed'].map((item) => <option key={item}>{item}</option>)}</select><button className="link" onClick={() => setFilters({ organization: '', category: '', venue: '', eventType: '', date: '', month: '', status: '' })}>Reset filters</button></section>;
}
...
function EventEditor({ event, store, user, close, save, remove }: { event: CalendarEvent; store: Store; user: User; close: () => void; save: (event: CalendarEvent) => void; remove: (event: CalendarEvent) => void }) {
  ...
  return <aside className="editor"><header><div><span className="eyebrow">Event schedule</span><h2>{store.events.some((item) => item.id === draft.id) ? 'Edit event' : 'Post event'}</h2></div><button onClick={close}>×</button></header>
    <div className="editor-body">
      ...
      <div className="two"><label>Category<select value={draft.category_id} onChange={(event) => set('category_id', event.target.value)}>{store.categories.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Status<select value={draft.event_status} onChange={(event) => set('event_status', event.target.value)}>{['planned', 'finalized', 'postponed', 'cancelled', 'completed'].map((item) => <option key={item}>{item}</option>)}</select></label></div><div className="two"><label>Privacy<select value={draft.privacy_level} onChange={(event) => set('privacy_level', event.target.value)}><option value="basic">Public details</option><option value="internal">Internal only</option></select></label><label>Expected attendees<input type="number" min="1" value={draft.expected_attendees} onChange={(event) => set('expected_attendees', Number(event.target.value))} /></label></div>
      <label>Short public description<textarea value={draft.public_description} onChange={(event) => set('public_description', event.target.value)} /></label><label>Purpose<textarea value={draft.purpose} onChange={(event) => set('purpose', event.target.value)} /></label><div className="two"><label>Contact person<input value={draft.contact_person} onChange={(event) => set('contact_person', event.target.value)} /></label><label>Contact email or number<input value={draft.contact_info} onChange={(event) => set('contact_info', event.target.value)} /></label></div><label>Private organization notes<textarea value={draft.private_notes} onChange={(event) => set('private_notes', event.target.value)} /></label>
...
function Dashboard({ store, events }: { store: Store; events: CalendarEvent[] }) { return <Workspace title="Dashboard"><div className="metrics"><Metric label="Visible events" value={events.length} /><Metric label="Upcoming programs" value={events.filter((item) => eventIsActive(item) && new Date(item.end_time) >= new Date()).length} /><Metric label="Conflict warnings" value={events.filter((item) => item.conflict_event_ids?.length).length} /><Metric label="Organizations" value={store.organizations.length} /><Metric label="Blocked dates" value={store.blockedTimes.length} /><Metric label="Announcements" value={store.announcements.length} /><Metric label="Open concerns" value={store.concerns.filter((item) => !['resolved', 'rejected'].includes(item.status)).length} /></div></Workspace>; }
...
function Notifications({ store, user }: { store: Store; user: User }) {
  const own = user.role === 'super_admin' ? store.events : store.events.filter((item) => item.organization_id === user.organization_id);
  const rows = [...store.announcements.map((item) => ({ title: `Announcement: ${item.title}`, detail: item.content, date: item.posted_at })), ...store.blockedTimes.map((item) => ({ title: `Blocked date: ${item.title}`, detail: item.reason || 'University restriction', date: item.start_time })), ...own.filter((item) => item.conflict_event_ids?.length).map((item) => ({ title: `Conflict warning: ${item.title}`, detail: `${item.conflict_event_ids!.length} overlapping event(s)`, date: item.updated_at })), ...store.concerns.filter((item) => item.admin_response && (user.role === 'super_admin' || item.organization_id === user.organization_id)).map((item) => ({ title: `Concern response: ${item.title}`, detail: item.admin_response!, date: item.updated_at }))].sort((a, b) => b.date.localeCompare(a.date));
  return <Workspace title="Notifications">{rows.length ? rows.map((item, index) => <article className="list-card" key={`${item.title}-${index}`}><strong>{item.title}</strong><p>{item.detail}</p><small>{item.date}</small></article>) : <p>No notifications right now.</p>}</Workspace>;
}
function Announcements({ store, user, persist }: { store: Store; user: User; persist: (store: Store, message: string) => void }) {
  function add() { const title = prompt('Announcement title:')?.trim(), content = prompt('Announcement message:')?.trim(), priority = prompt('Priority: normal, important, or urgent', 'normal')?.trim() || 'normal', expires = prompt('Expiry date (YYYY-MM-DD), or leave blank:')?.trim(); if (!title || !content) return; const item: Announcement = { id: createId(), title, content, priority, posted_by: user.full_name, posted_at: new Date().toISOString(), expires_at: expires ? `${expires}T23:59:59` : undefined }; void persist({ ...store, announcements: [...store.announcements, item] }, 'Announcement posted.'); }
  return <Workspace title="Announcements">{user.role === 'super_admin' && <button className="primary" onClick={add}>+ Post announcement</button>}{[...store.announcements].sort((a, b) => (a.priority === 'urgent' ? -1 : 1) - (b.priority === 'urgent' ? -1 : 1)).map((item) => <article className={`list-card announcement-${item.priority}`} key={item.id}><strong>{item.title}</strong><p>{item.content}</p><small>{item.priority} · posted by {item.posted_by}{item.expires_at ? ` · expires ${item.expires_at.slice(0, 10)}` : ''}</small>{user.role === 'super_admin' && <button className="danger-link" onClick={() => void persist({ ...store, announcements: store.announcements.filter((row) => row.id !== item.id) }, 'Announcement deleted.')}>Delete</button>}</article>)}</Workspace>;
}
function Concerns({ store, user, persist }: { store: Store; user: User; persist: (store: Store, message: string) => void }) {
  ...
  function respond(item: Concern) { const admin_response = prompt('Admin response:', item.admin_response || '')?.trim(); if (admin_response === undefined) return; void persist({ ...store, concerns: store.concerns.map((row) => row.id === item.id ? { ...row, admin_response, status: 'in_review', updated_at: new Date().toISOString() } : row) }, 'Concern response saved.'); }
  return <Workspace title="Concerns">{user.role === 'organization_manager' && <button className="primary" onClick={add}>+ Raise concern</button>}{items.map((item) => <article className="list-card" key={item.id}><strong>{item.title}</strong><p>{item.description}</p><small>{item.organization_name} · {item.status}</small>{item.admin_response && <p>Admin response: {item.admin_response}</p>}{user.role === 'super_admin' && <><button onClick={() => respond(item)}>Respond</button><button onClick={() => void persist({ ...store, concerns: store.concerns.map((row) => row.id === item.id ? { ...row, status: 'resolved', updated_at: new Date().toISOString() } : row) }, 'Concern resolved.')}>Resolve</button><button className="danger-link" onClick={() => void persist({ ...store, concerns: store.concerns.map((row) => row.id === item.id ? { ...row, status: 'rejected', updated_at: new Date().toISOString() } : row) }, 'Concern rejected.')}>Reject</button></>}</article>)}</Workspace>;
}
