      ...
      try {
        ...
        if (status === 'approved' && row.revision_of) {
          ...
          if (original && plfIsCancellationRequest(row)) {
            ...
            await plfPatchCalendarItem(original.id, { ...plfFullSchedulePayload(row), created_by: original.created_by || row.created_by, created_at: original.created_at || row.created_at });
          }
        }
...
function plfBindOrgScheduleFallback() {
  ...
  document.addEventListener('click', (event) => {
    if (event.target?.id === 'agreementSubmitButton' && plfIsOrgUser()) {
      ...
      if (snapshot) window.setTimeout(() => plfPostCalendarItem(plfOrgDbRow(snapshot)).then(() => plfToast(snapshot.revision_of ? 'Schedule change submitted for admin approval.' : 'Schedule saved to database.', 'success')).catch((error) => plfToast(`Org schedule save failed: ${error.message}`, 'error')), 700);
    }
